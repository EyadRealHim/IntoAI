...
import loadImage, { Img } from "./utils/loadImage.js";
...
export function background(context: CanvasRenderingContext2D, game: Game) {
...
export function conveyorBelt(context: CanvasRenderingContext2D, game: Game) {
  ...
  for (
    let x = 0;
    x <= game.viewWidth + pulleySize;
    x += pulleySize + pulleyMargin
  ) {
...
export function displayScore(context: CanvasRenderingContext2D, game: Game) {
...
export function drawImage(
  ...
  context.drawImage(
    image,
    shape.center ? -shape.width / 2 : 0,
    shape.center ? -shape.height / 2 : 0,
    shape.width,
    shape.height
  );
  context.restore();
