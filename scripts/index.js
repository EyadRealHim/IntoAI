...
const canvas = assertsValue(document.getElementById("gameCanvas"), "gameCanvas is not found.");
...
function frame() {
    ...
    if (game.laser) {
        if (game.laser.loading)
            draw.displayWarn(context, game, game.laser);
        draw.laser(context, game, game.laser);
    }
    if (!game.isOnStartState)
        draw.displayScore(context, game);
    if (game.gameover) {
...
function update() {
    if (game.gameover || game.pause)
        return;
    const absolute = (e) => e * config.GAME_SPEED;
    if (game.progressCounter % absolute(config.WRENCH_GENERATE_TIME) ==
        config.GAME_SPEED) {
        generateWrench(game);
    }
    if (game.animationCounter % config.BIRD_ANIMATION_SPEED == 0) {
        birdSprite.next();
        if (birdSprite.index == 4)
            audios.flap();
    }
    if (!game.laser &&
